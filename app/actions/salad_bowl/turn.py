...
def start_turn(game_id, round_id):
    ...
    if form.validate_on_submit(
        ...
        current_users_team_id_q = db.session.query(PlayerTeam.team_id)
        current_users_team_id_q = current_users_team_id_q.join(Team)
        current_users_team_id = current_users_team_id_q.filter(Team.game_id == game_id,
                                                               PlayerTeam.player_id == g.current_user.id).scalar()

        new_turn = Turn(round_id=round_id,
                        team_id=current_users_team_id,
                        player_id=g.current_user.id,
                        started_at=datetime.utcnow(),
                        expected_complete_at=datetime.utcnow() +
                        timedelta(seconds=int(form.turn_length.data)))  # TODO dont get this value from the form
        db.session.add(new_turn)
        db.session.commit()

        return True, redirect(url_for('.view_turn', game_id=game_id, round_id=round_id, turn_id=new_turn.id))

    return False, render_template('salad_bowl/actions/start_turn.html',
                                  form=form,
...
def word_guessed(game_id, round_id, turn_id):
    ...
    if form.validate_on_submit():
        ...
        guessed_word = GuessedWord(word_id=form.word_id.data,
                                   round_id=round_id,
                                   team_id=current_users_team_id,
                                   player_id=g.current_user.id)
        ...
        unguessed_words_q = unguessed_words_q.join(GuessedWord,
