...
def sign_in():
    ...
    if request.method == 'POST':
        ...
        if not user_for_login:
            error_msg = 'User not found or incorrect passcode given.'
    ...
    return render_template('actions/sign_in.html',
                           form=form,
                           action_url=url_for('.sign_in', next_url=request.args.get('next_url')),
                           error_msg=error_msg)
